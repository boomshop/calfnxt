export const expanderInfo = {
  ...
  mode:
    'How the detector measures level. Peak = snappy, follows hits. RMS = smoother average — less chatter. Opto = photocell-like ballistics that soften attack/release as reduction deepens.',
  link:
    'How L/R feed the detector. Max = louder channel wins. Avg = mean of both. Mid = Mid/sum only — useful when you want expansion driven by the phantom center.',
  threshold:
  ...
  knee:
    'Softens the open threshold and the landing into Range. 0 = hard corners. Higher = gentler engagement and less abrupt floor.',
  attack:
    'How fast the detector envelope rises. Short = reacts to hits quickly (opens/tracks faster). Longer = ignores brief spikes.',
  hold:
    'Keeps the expander open for this long after the detector falls below Rel Thresh, before closing begins. Stops chatter on gaps without slowing the release itself. 0 = close immediately.',
  release:
  ...
  gr:
    'How many dB the expander is currently turning the signal down. Spikes on quiet gaps are normal; a meter stuck deep means you’re continuously gating the body.',
};

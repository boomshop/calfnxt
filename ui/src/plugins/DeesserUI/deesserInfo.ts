...
export const deesserInfo = {
  ...
  channel:
    'Which stereo path the detector and reduction act on (same path). Stereo = both channels. Left / Right = treat one side; in Split the other is LR-allpass matched so the stereo field stays flat when GR=1. Mid / Side = same on centre or width. Listen solos the detector (Side images L/−R). Split and Wide still apply on the selected path. Encode → detect/process → decode.',

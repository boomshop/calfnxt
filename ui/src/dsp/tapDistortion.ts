...
export type TapCoeffs = {
  ...
  an: number;
  pwrq: number;
  /** Same scale as DSP TapDistortion::kAsymScale. */
  asymmetry: number;
};

const ASYM_SCALE = 0.35;
...
export function makeTapCoeffs(
  ...
  return {
    ...
    an,
    pwrq,
    asymmetry: Math.max(-1, Math.min(1, asymmetry)),
  };
}

export function shapeStatic(x: number, c: TapCoeffs): number {
  x = x + c.asymmetry * ASYM_SCALE;
  if (x >= 0)
...
export function sampleTransferCurve(
  ...
  const c = makeTapCoeffs(blend, drive, asymmetry);

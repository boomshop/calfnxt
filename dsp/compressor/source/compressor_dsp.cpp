...
void CompressorPlugin::resetProcessing()
{
  ...
  grMeterFallDbPerSample_ = 20.f / static_cast<float>(sampleRate_);
  grMeterDb_ = 0.f;
  grMeterDbPub_.store(0.f, std::memory_order_relaxed);
  {
...
void CompressorPlugin::updateGrMeter(float grLin)
{
  // Positive GR amount in dB. Instant attack, linear fall toward 0 (AUX-style).
  ...
  if (grAmt > grMeterDb_)
    ...
    grMeterDb_ = std::max(0.f, grMeterDb_ - grMeterFallDbPerSample_);

  grMeterDbPub_.store(grMeterDb_, std::memory_order_relaxed);
}
...
int CompressorPlugin::takeGainReductionDb(float* out, int maxOut)
{
  ...
  out[0] = -grMeterDbPub_.load(std::memory_order_acquire);
  return 1;
}

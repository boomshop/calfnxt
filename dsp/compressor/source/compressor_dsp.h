...
private:
  ...
  void updateGrMeter(float grLin);
  ...
  // GR meter ballistics (audio thread): instant attack, fixed dB/s fall toward 0.
  float grMeterDb_ = 0.f;
  float grMeterFallDbPerSample_ = 0.f;
  std::atomic<float> grMeterDbPub_ {0.f};
  std::mutex vizMutex_;
